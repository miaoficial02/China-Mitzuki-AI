...
async function handler(conn, { message, args }) {
    ...
    if (!query) {
        return conn.sendMessage(message.key.remoteJid, { text: '💭 *Shizuka* te recuerda: Por favor, ingresa un término de búsqueda para la canción. 🎶' });
    }
    ...
    try {
        ...
        await conn.sendMessage(message.key.remoteJid, { text: '⚠️ *Shizuka* encontró un error al intentar descargar el archivo. Intenta con otro término de búsqueda. ❌' });
    }
}
...
async function sendAudioAsFile(conn, message, audioUrl, audioTitle) {
    ...
    try {
        ...
        await conn.sendMessage(message.key.remoteJid, { text: '⚠️ *Shizuka* no pudo enviar el archivo de audio. Intenta nuevamente. ❌' });
    }
}
